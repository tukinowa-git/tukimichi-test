/* book.v11.4.js — 月みちて、山となる。 PC版 スクリプト本体
   2026-09-17 / 10見開き構成（CSS は book.v11.4.css）
...
addEventListener("pageshow",e=>{
  ...
  /* ▼㉔ v11.4：詳細ページのナビから「戻る」で復元されたとき、行き先が指定されていればめくる */
  try{const one=sessionStorage.getItem("tkgo");
    if(one){sessionStorage.removeItem("tkgo");
      const a=one.split("|"),t=+a[0];
      if(Date.now()-(+a[1]||0)<3e4&&t>=1&&t<TS&&t!==cs)setTimeout(()=>go(t),60)}
  }catch(err){}});
