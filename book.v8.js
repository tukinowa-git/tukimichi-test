/* book.v8.js — 月みちて、山となる。 PC版 スクリプト本体
   2026-09-11 / 10見開き構成（CSS は book.v8.css）
     1) ページ遷移エンジン（GSAP依存）
     2) Instagramフィード連携（右=最新1件／左=次の6件）
     3) お問い合わせフォーム（入力の反応・確認・GASへの送信）
     4) 地図（近づいたら読み込む／クリックで操作）
...
function boot(){
  ...
  l.forEach(function(c){
    c.addEventListener("click",function(){load();c.classList.add("on")});
