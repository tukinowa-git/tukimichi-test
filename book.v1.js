/* book.v1.js — 埋め込みコードから外部化したスクリプト本体
   1) ページ遷移エンジン（GSAP依存）
   2) SNSフィード連携
   ※ 読み込み側のブートストラップ（画像パス展開・初期表示解除）は含みません */
...
if(pp.some(p=>!p)||typeof gsap=="undefined")return setTimeout(boot,60);
/* === 調整パラメータ === */
...
let tr=0,iv=0,cs=0,an=0,lk=1,wa=0,tx=0,ty=0;
...
function intro(){
if(iv)return;iv=1;
const lg=D.querySelectorAll(".ilh img"),ti=D.querySelector(".itl"),ln=D.querySelectorAll(".itx span"),tl=gsap.timeline();
...
addEventListener("wheel",e=>{e.preventDefault();if(an||lk)return;wa+=e.deltaY;if(Math.abs(wa)>70){turn(wa>0?1:-1);wa=0}},{passive:false});
addEventListener("touchstart",e=>{tx=e.touches[0].clientX;ty=e.touches[0].clientY},{passive:true});
addEventListener("touchend",e=>{if(an||lk)return;const x=tx-e.changedTouches[0].clientX,y=ty-e.changedTouches[0].clientY;if(Math.abs(x)>50&&Math.abs(x)>Math.abs(y))turn(x<0?1:-1);else if(Math.abs(y)>50)turn(y>0?1:-1)},{passive:true});
addEventListener("keydown",e=>{if(an||lk)return;if(["ArrowRight","ArrowDown"," "].includes(e.key)){e.preventDefault();turn(1)}if(["ArrowLeft","ArrowUp"].includes(e.key)){e.preventDefault();turn(-1)}});
...
function init(){load();setInterval(load,CACHE_MIN*6e4)}
