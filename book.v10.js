/* book.v10.js — 月みちて、山となる。 PC版 スクリプト本体
   2026-09-14 / 10見開き構成（CSS は book.v10.css）
     1) ページ遷移エンジン（GSAP依存）
   ...
   ■ v9 → v10 の変更（1章のめくりだけ。2〜4章は v9 のまま）
     ㉑ Safari でページの残像が出る問題の対策。
        原因：紙9枚はすべて同じ奥行き（z=0）に並んでいるため、前後関係を
        z-index だけで決めています。Chrome / Edge は z-index の変更をすぐ
        反映しますが、WebKit は will-change で常時レイヤ化された要素の
        並び替えを一拍遅れて適用することがあり、めくり終わった紙が
        次の見開きの上に残って見えていました。
        対策：紙ごとに 0.08px きざみの「本当の奥行き」を与え、z-index に
        頼らなくても順序が決まるようにしました。奥行きの差は最大 0.7px、
        遠近法は 2800px なので見た目は一切変わりません。
     ㉒ will-change:transform を常時ではなく「いまめくっている紙」だけに。
        9枚ぶんの合成レイヤ（画面の半分×9）を抱え続けずに済みます。
...
/* ▼㉑ 紙1枚あたりの奥行き（px）。0.08 × 9枚 = 最大 0.72px。
   遠近法 2800px に対して 0.03% 未満なので、大きさの変化は見えません。
   めくっていない紙は「若い番号ほど手前」、めくり終えた紙は「新しいほど手前」。
   GSAP は translate を先に書き出すので、z は回転に関係なく
   つねに「画面手前が＋」になります。 */
const ZU=.08;
function bz(i,f){return f?(i+1)*ZU:(T-i)*ZU}
...
function z(){pp.forEach((p,i)=>{const f=i<cs;p.style.zIndex=f?i*10+5:(T-i)*10;gsap.set(p,{z:bz(i,f)})});fc()}
...
function flip(d,dur,cb){
  ...
  /* ▼⑮ めくっている間は両面を出す ／ ⑱ ページ内の入力欄にフォーカスが残っていたら外す */
  p.classList.remove("tn","st");
  const ae=D.activeElement;if(ae&&ae!==D.body&&ae.closest&&ae.closest(".pp"))try{ae.blur()}catch(e){}
  /* ▼㉒ めくっている間だけレイヤー化する。終わったら手放す */
  p.style.zIndex=1e3;p.style.willChange="transform";
  /* ▼㉑ 着地したときの奥行き。めくり終われば「新しいほど手前」に入る */
  const fz=d>0?bz(cs,1):bz(cs,0);
  gsap.timeline({onComplete(){if(d>0)cs++;p.style.willChange="";z();n();cb&&cb()}})
  .to(p,{rotationY:d>0?180:0,duration:dur,ease:"power2.inOut"},0)
  .to(p,{z:30,duration:h,ease:"sine.inOut"},0)
  .to(p,{z:fz,duration:h,ease:"sine.inOut"},h)
...
R.classList.add("ld");
pp.forEach((p,i)=>{p.style.zIndex=(T-i)*10;gsap.set(p,{rotationY:0,z:bz(i,0)})});fc();
...
if(DP){
  ...
  pp.forEach((p,i)=>gsap.set(p,{rotationY:i<cs?180:0,z:bz(i,i<cs)}));
