/* book.v6.js — 月みちて、山となる。 PC版 スクリプト本体
   2026-09-07 / 10見開き構成
     1) ページ遷移エンジン（GSAP依存）
...
function leave(u){if(R.classList.contains("lv"))return;
  /* ▼⑫ いま開いている見開きをURLに残す。戻ったときここから再開するため。
         replaceState なので履歴は増えません */
  try{if(cs>=1)history.replaceState(null,"","#s"+cs)}catch(e){}
  R.classList.add("lv");
...
const DP=(()=>{const m=/^#s(\d+)$/.exec(location.hash||"");if(!m)return 0;
  const v=parseInt(m[1],10);return(v>=1&&v<=TS-1)?v:0})();
