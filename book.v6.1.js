/* book.v6.1.js — 月みちて、山となる。 PC版 スクリプト本体
   2026-09-08 / 10見開き構成
   book.v6.js の小改訂。CSS（book.v6.css）は変更なしのため据え置きです。
     1) ページ遷移エンジン（GSAP依存）
   ...
   ■ v6.1 での変更
     ⑫' URLを一切変えずに、戻り先を覚えるようにしました。
        v6ではURLに #s3 を書いていましたが、戻ったあともURLに残り、
        別の見開きへ移っても古い番号のままになる問題がありました。
        v6.1では次の3つを使い分けます。URLは常に / のままです。

          (1) sessionStorage（同じタブの中だけで生きる控え）
              めくるたびに、いまの見開き番号を控えます。
          (2) navigation.type === "back_forward"
              「戻る／進む／横スワイプで来た」ことをブラウザが教えてくれます。
              このときだけ(1)の控えを読んで、その見開きから開きます。
          (3) 詳細ページのナビからの一回限りの指示（sessionStorage の tkgo）
              「たべるへ行きたい」という指示を置いてからトップへ移動します。

        ハッシュ付きURL（/#s4）で直接開かれた場合も従来どおり動きます。
        新しいタブで開いたときのために残していますが、こちらから書くことは
        ありません。読み取ったあとURLからは消します。
        sessionStorage が使えない環境（プライベートモード等）でのみ、
        従来どおりURLに印を残す動きに切り替わります。

   ■ v6.0 での変更（v5の①〜⑪はすべてそのまま残しています）
     ⑫ 離脱するときだけURLにハッシュを書き込む。
        「詳細を見る」を押した瞬間に history.replaceState で #s3 を残すので、
        ブラウザの戻るボタンや2本指スワイプで戻ったときに、必ずその見開きから
        再開します。通常のめくりではURLを変えないので、アドレスバーは普段 / のままです。
        （bfcache が効かない環境でも確実に戻ることを実測で確認済み）
     ⑬ 横方向のホイールは横取りしない。
        macOSの2本指スワイプやマウスの横スクロールは横方向のホイールとして
        届きます。v5では #root 内のホイールを無条件に preventDefault していたため、
        ブラウザの「戻る」ジェスチャを潰していました。めくりに使うのは deltaY だけです。
     ⑭ 離脱の待ち時間を 380ms → 190ms に短縮。

   ■ v5 で追加した3つのしくみ（v4の①〜⑧はすべてそのまま残しています）
     ⑨ ディープリンク起動。URLが #s3 のように来たときは、ローディング演出を
        飛ばしてその見開きを開いた状態から始める。詳細ページから戻ったときに
        先頭の5.2秒をもう一度見せないため。
        ?p=3 ではなく # を使うのは、? 付きURLが検索エンジンに別ページとして
        拾われるのを避けるため（正規化の手当てが要らない）。
     ⑩ 別ページへの離脱。data-leave の付いたリンクは、中身だけを送り出してから
        遷移する。⌘/Ctrl/Shift/中クリックと target 付きは横取りしない。
        マウスが乗った時点で遷移先を先読みする。
     ⑪ 戻るボタン対策。bfcacheから復元されたとき、⑩で消した中身が消えたままに
        ならないよう静かに戻す。これが無いと「戻ると真っ白」になる。
